...
Here are some common ideas that appear that relate to PGFs:

\begin{example*}[Common PGF Patterns]
\begin{itemize}
\item Independent sums: \[G_{X+Y}=G_XG_Y\]
\item First-step analysis: \[G(s)=\text{immediate contribution}+\text{restart contribution}\]
\item Moments: \[E[X]=G'(1),\qquad E[X^2]=G''(1)+G'(1).\]
\end{itemize}
\end{example*}
...
\begin{enumerate}
\item A random variable $X$ has PGF \[G_X(s)=\frac{1+s+s^2+s^3}{4}.\] Find \[\mathbb{E}[X],\qquad \operatorname{Var}(X).\]
\item Three fair $10$-sided dice are rolled. Find the probability that the sum is congruent to $1 \pmod{4}$.
\item A biased coin has probability $p$ of heads. It is flipped until the first head appears. Let $X$ be the total number of flips. Find the:
\begin{itemize}
...
\item $\mathbb{E}[X^3]$.
\end{itemize}
\item A particle starts at $0$. Each step it moves right with probability $p$ and left with probability $p$ for $p < \tfrac{1}{2}$.
Find the expected time to first reach $2$.
\item For the random walk in the previous problem, find \[\mathbb{E}[T^2].\]
\item A random variable $X$ satisfies \[G_X(s)=\frac13(1+s+s^2G_X(s)).\] Find \[\mathbb{E}[X] \qquad\text{and}\qquad \operatorname{Var}(X).\]
\item A plant reproduces as follows: each plant produces $0$, $1$, or $2$
